...
// 각 Player들의 점수를 계산하고, 점 수가 제일 높은 사람을 출력하는 코드이다.
...
class Exercise11_13 {
    ...
    {
        SutdaDeck deck = new SutdaDeck(); deck.shuffle();
        Player[] pArr = {
        ...
        TreeMap rank = new TreeMap(new Comparator(){ public int compare(Object o1, Object o2) {
/*
...
*/
            ...
            for(int i=0; i < pArr.length;i++) { Player p = pArr[i]; rank.put(p, deck.getPoint(p));
        ...
        class SutdaDeck
        {
            ...
            void registerJokbo() { jokbo.put("KK", 4000);
            ...
            int getPoint(Player p) { if(p==null) return 0;

                SutdaCard c1 = p.c1; SutdaCard c2 = p.c2;
                Integer result = 0; if(c1.isKwang && c2.isKwang) {
            ...
            SutdaCard pick() throws Exception { SutdaCard c = null;

                if(0 <= pos && pos < CARD_NUM) { c = cards[pos]; cards[pos++] = null;
                } else {
            ...
            void shuffle() {
                for(int x=0; x < CARD_NUM * 2; x++) {
                    int i = (int)(Math.random() * CARD_NUM); int j = (int)(Math.random() * CARD_NUM);

                    SutdaCard tmp = cards[i]; cards[i] = cards[j]; cards[j] = tmp;
                }
            }
        } // SutdaDeck

        class Player { String name; SutdaCard c1; SutdaCard c2;
